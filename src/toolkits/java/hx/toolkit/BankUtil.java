...
public class BankUtil {
    ...
    public static String getName(String cardNo) {
        String charBin = cardNo.substring(0, 6);
        int bin = 0, index = 0;
        bin = Integer.valueOf(charBin);
        index = binarySearch(bankBin, bin);
        if (index == -1 || index > bankName.length) return "";
...
//    从不含校验位的银行卡卡号采用 Luhm 校验算法获得校验位
    private static char getBankCardCheckCode(String nonCheckCodeCardId) {
        //如果传的不是数据返回N
        if (nonCheckCodeCardId == null || nonCheckCodeCardId.trim().length() == 0 || !nonCheckCodeCardId.matches("\\d+")) return 'N';
